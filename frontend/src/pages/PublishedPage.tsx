...
import { CheckCircle, Globe, Copy, ExternalLink, Code, Monitor, Check, ChevronRight, Key, RefreshCw } from 'lucide-react';
import { getProjects, type Project, type LandingPage } from './ProjectsPage';

const uid = () => Math.random().toString(36).slice(2, 18).toUpperCase();

type Integration = 'wordpress' | 'script' | 'embed';
...
const WordPressIntegration = ({ project, pageSlug }: { project: Project, pageSlug: string }) => {
  ...
  const script = token
    ? `<!-- PageBuilder.ai Integration -->\n<script>\n  window.__PB_TOKEN__ = "${token}";\n  window.__PB_PAGE__ = "${pageSlug}";\n</script>\n<script src="https://cdn.pagebuilder.ai/loader.js" async defer></script>`
    : '';

  const copyToken = () => { navigator.clipboard.writeText(token); setCopiedToken(true); setTimeout(() => setCopiedToken(false), 2000); };
    ...
    <div>
      ...
      <div style={{ marginBottom: 16, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          ...
          <a href="https://wordpress.org/plugins/" target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: '#4f46e5', textDecoration: 'none', fontWeight: 600 }}>
            Get Plugin <ExternalLink size={12} />
          </a>
        </div>
        <p style={{ fontSize: 13, color: '#6b7280', margin: 0, lineHeight: 1.6 }}>
          Go to your WordPress dashboard → <strong>Plugins → Add New</strong> → Search for <strong>"PageBuilder AI"</strong> → Install &amp; Activate.
        </p>
      </div>
      ...
      <div style={{ marginBottom: 16, background: '#fff', border: `1px solid ${token ? '#bbf7d0' : '#e5e7eb'}`, borderRadius: 10, padding: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: token ? 12 : 0 }}>
          ...
          {!token && (
            <button style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#4f46e5', color: '#fff', border: 'none', borderRadius: 7, padding: '7px 14px', fontSize: 12, fontWeight: 600 }}>
              <Key size={12} /> View Token
            </button>
          )}
        </div>
        {token && (
        ...
        {token ? (
          <>
            <p style={{ fontSize: 12, color: '#6b7280', margin: '0 0 10px', lineHeight: 1.6 }}>
              Go to <strong>Appearance → Theme Editor → header.php</strong> (or use <em>Insert Headers and Footers</em> plugin) and paste this before <code style={{ background: '#fee2e2', color: '#991b1b', padding: '1px 5px', borderRadius: 4 }}>&lt;/head&gt;</code>:
            </p>
          ...
          <p style={{ fontSize: 13, color: '#9ca3af', margin: 0, fontStyle: 'italic' }}>Generate your token first to see the script.</p>
        )}
      </div>
    </div>
  );
...
const ScriptIntegration = ({ project, pageSlug }: { project: Project, pageSlug: string }) => {
  ...
  const snippet = `<script src="https://cdn.pagebuilder.ai/embed.js?token=${project.token}&page=${pageSlug}" async></script>`;
  ...
  return (
    <div>
      <p style={{ fontSize: 14, color: '#6b7280', margin: '0 0 16px', lineHeight: 1.6 }}>
        Paste this script tag anywhere in your website's HTML — ideally before the closing <code style={{ background: '#f1f5f9', padding: '1px 5px', borderRadius: 4 }}>&lt;/body&gt;</code> tag.
        It will automatically render your published page at the target location.
      </p>
      ...
      <div style={{ marginTop: 16, background: '#fefce8', border: '1px solid #fde68a', borderRadius: 8, padding: '12px 16px', fontSize: 12, color: '#92400e' }}>
        💡 <strong>Tip:</strong> This script loads your page asynchronously and won't slow down your website.
      </div>
    </div>
  );
};
...
// ─── Embed iFrame ─────────────────────────────────────────────────────
const EmbedIntegration = ({ publishUrl }: { publishUrl: string }) => {
  const [copied, setCopied] = useState(false);
  const code = `<iframe\n  src="${publishUrl}"\n  width="100%"\n  height="700"\n  frameborder="0"\n  scrolling="auto"\n  allowfullscreen\n></iframe>`;
  ...
  return (
    <div>
      <p style={{ fontSize: 14, color: '#6b7280', margin: '0 0 16px', lineHeight: 1.6 }}>
        Embed your published landing page directly on any HTML page using this iframe code. Works on any website or CMS.
      </p>
      <div style={{ position: 'relative', background: '#0f172a', borderRadius: 10, padding: 18 }}>
        <pre style={{ color: '#a5f3fc', fontSize: 12, fontFamily: 'monospace', margin: 0, lineHeight: 1.7 }}>{code}</pre>
        <button onClick={copy} style={{ position: 'absolute', top: 12, right: 12, display: 'flex', alignItems: 'center', gap: 5, background: copied ? '#22c55e' : '#fff', color: copied ? '#fff' : '#374151', border: 'none', borderRadius: 6, padding: '5px 12px', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
          {copied ? <Check size={12} /> : <Copy size={12} />} {copied ? 'Copied!' : 'Copy'}
        </button>
      </div>
      <div style={{ marginTop: 16, background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 8, padding: '12px 16px', fontSize: 12, color: '#1e40af' }}>
        💡 <strong>Tip:</strong> Adjust the <code>height</code> attribute to match your page length, or use <code>height="100%"</code> on a full-page container.
      </div>
    </div>
  );
};
...
// ─── Main PublishedPage ───────────────────────────────────────────────
const PublishedPage = () => {
  const [integration, setIntegration] = useState<Integration>('wordpress');
  ...
  const project = getProjects().find(p => p.id === projId);
  const pageDef = project?.pages.find(p => p.id === pageId);

  if (!project || !pageDef) {
    return (
      <div style={{ padding: 40, textAlign: 'center', fontFamily: 'Inter' }}>
        <h2 style={{ color: '#0f172a' }}>Loading your page...</h2>
      </div>
    );
  }

  const liveUrl = pageDef.publishUrl || `https://pub.ppcbuilder.io/p/${project.id.slice(-6)}/${pageDef.slug}`;

  const copyUrl = () => { navigator.clipboard.writeText(liveUrl); setUrlCopied(true); setTimeout(() => setUrlCopied(false), 2000); };
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 28 }}>
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
            ...
            <button onClick={copyUrl} style={{ border: '1px solid #e5e7eb', borderRadius: 8, padding: '9px 12px', background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', color: urlCopied ? '#22c55e' : '#6b7280' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 24 }}>
          ...
          {[{ label: 'Status', value: '● Published', color: '#22c55e' }, { label: 'Total Views', value: pageDef.views.toString() }, { label: 'Leads', value: pageDef.leads.toString() }, { label: 'Published', value: 'Just now' }].map((s) => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div>
            ...
            <div style={{ background: '#f8fafc', borderRadius: 10, padding: 22, border: '1px solid #e5e7eb' }}>
              <WordPressIntegration project={project} pageSlug={pageDef.slug || "page"} />
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ background: '#f8fafc', borderRadius: 10, padding: 22, border: '1px solid #e5e7eb' }}>
              <ScriptIntegration project={project} pageSlug={pageDef.slug || "page"} />
            </div>
          </div>
        </div>
      </div>
