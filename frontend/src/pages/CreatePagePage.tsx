...
import { ArtifactLoader } from "@/components/ui/ArtifactLoader";
...
const CreatePagePage = () => {
  ...
  const [selectedStyle, setSelectedStyle] = useState<string>("✨ Modern & Clean");
  ...
  const validateForm = async () => {
    ...
    let currentName = pageName.trim();
    if (!currentName) {
      if (aiPrompt.trim()) {
        currentName = aiPrompt.trim().slice(0, 35).replace(/[^\w\s]/gi, '').trim() || "AI Landing Page";
      } else {
        currentName = "AI Landing Page";
      }
      setPageName(currentName);
      setPageNameError("");
    }

    let currentSlug = pageSlug.trim();
    if (!currentSlug) {
      currentSlug = autoSlug(currentName) + "-" + Date.now().toString(36).slice(-4);
    ...
    if (activeMethod === "ai") {
      if (!aiPrompt.trim()) {
        setMethodError("Please describe your page or select a preset prompt below.");
        isValid = false;
      } else {
        setMethodError("");
      }
    ...
    if (!isSlugVerified) {
      ...
      if (hasError) {
        const uniqueSlug = normalizedSlug + "-" + Math.floor(Math.random() * 1000);
  ...
  const handleCreate = async () => {
    ...
    let detectedUrl = "";
    let effectiveAiPrompt = aiPrompt;
    const urlRegex = /(https?:\/\/[^\s]+)/g;
    const urls = aiPrompt.match(urlRegex);
    ...
    if (detectedUrl) {
      ...
      try {
        const scrapeRes = await aiApi.extractProject(detectedUrl);
        if (scrapeRes?.data) {
          const brandData = scrapeRes.data;
          if (brandData.primaryColor) finalPrimaryColor = brandData.primaryColor;
          if (brandData.secondaryColor) finalSecondaryColor = brandData.secondaryColor;
          if (brandData.projectLogo) finalLogoUrl = brandData.projectLogo;

          // Inject keywords and description into the AI prompt so the generator actually uses them!
          if (brandData.keywords?.length || brandData.projectDesc) {
            const extraContext = `\n\n[SCRAPED BRAND DATA]\nKeywords: ${brandData.keywords?.join(', ') || 'N/A'}\nDescription: ${brandData.projectDesc || 'N/A'}\n`;
            if (!effectiveAiPrompt.includes("[SCRAPED BRAND DATA]")) {
              effectiveAiPrompt += extraContext;
            }
          }
          ...
          tempProject = {
            ...
            scrapedData: {
              ...(project.scrapedData || {}),
              logo: finalLogoUrl,
              theme: brandData.themeSystem || {},
              fonts: brandData.fonts || {}
            },
            websiteProfile: {
              ...(project.websiteProfile || {}),
              identity: {
                ...(project.websiteProfile?.identity || {}),
        ...
        console.warn("Failed to scrape URL from prompt, falling back to defaults", e);
      } finally {
    ...
    if ((activeMethod === "template" && finalTemplateId) || isAiTemplatePath) {
      ...
      const finalLogo = finalLogoUrl || tempProject?.websiteProfile?.identity?.logoUrl || tempProject.logoUrl || tempProject.scrapedData?.logo || getProjectLogoUrl(tempProject);
      const logoHtml = finalLogo
        ...
        : `<span style="color: ${finalPrimaryColor}">${tempProject.name}</span>`;

      // ─── FINAL BRANDING INJECTION ───
      const themeData = tempProject.websiteProfile?.theme || tempProject.scrapedData?.theme || {};
      const fontsData = tempProject.websiteProfile?.fonts || tempProject.scrapedData?.fonts || {};
      ...
      const brandingCss = `
  :root, body {
  --primary: ${finalPrimaryColor || "#6366f1"};
  --secondary: ${finalSecondaryColor || "#4f46e5"};
  --primary-rgb: ${hexToRgbStr(finalPrimaryColor || "#6366f1")};
  --secondary-rgb: ${hexToRgbStr(finalSecondaryColor || "#4f46e5")};
  ...
  --button-gradient: linear-gradient(135deg, ${finalPrimaryColor || "#6366f1"}, ${finalSecondaryColor || "#4f46e5"});
}
...
`;
      ...
      enrichedContent = enrichedContent.replace(/PRIMARY_RGB_PLACEHOLDER/g, hexToRgbStr(finalPrimaryColor || "#6366f1"));
      enrichedContent = enrichedContent.replace(/SECONDARY_RGB_PLACEHOLDER/g, hexToRgbStr(finalSecondaryColor || "#4f46e5"));
      ...
      enrichedStyles = enrichedStyles
        .replace(/--primary\s*:\s*PRIMARY_COLOR_PLACEHOLDER/g, `--primary: ${finalPrimaryColor || "#6366f1"}`)
        .replace(/--secondary\s*:\s*SECONDARY_COLOR_PLACEHOLDER/g, `--secondary: ${finalSecondaryColor || "#4f46e5"}`)
        .replace(/--primary-rgb\s*:\s*PRIMARY_RGB_PLACEHOLDER/g, `--primary-rgb: ${hexToRgbStr(finalPrimaryColor || "#6366f1")}`)
        .replace(/--secondary-rgb\s*:\s*SECONDARY_RGB_PLACEHOLDER/g, `--secondary-rgb: ${hexToRgbStr(finalSecondaryColor || "#4f46e5")}`);
      ...
      const isPromptModified = effectiveAiPrompt.trim() !== defaultTplPrompt.trim();
      finalPromptForTemplate = isPromptModified ? effectiveAiPrompt : "";
...
${enrichedContent}
      ...
      basePayload = {
        ...
        primaryColor: finalPrimaryColor || tempProject?.primaryColor || "#7c3aed",
        secondaryColor: finalSecondaryColor || tempProject?.secondaryColor || "#6366f1",
        logoUrl: finalLogoUrl || tempProject?.websiteProfile?.identity?.logoUrl || tempProject.logoUrl || tempProject.scrapedData?.logo,
        industry: tempProject?.industry || "Service",
        subIndustry: tempProject?.subIndustry || tempProject?.scrapedData?.subIndustry || "Services",
        services: tempProject?.services || [],
    ...
    createPageMutation.mutate({
      ...
      secondaryColor: finalSecondaryColor || tempProject?.secondaryColor || "#6366f1",
      logoUrl: finalLogoUrl || tempProject?.websiteProfile?.identity?.logoUrl || tempProject.logoUrl || tempProject.scrapedData?.logo,
      industry: tempProject?.category || tempProject?.industry || "Service",
      subIndustry: tempProject?.subIndustry || tempProject?.scrapedData?.subIndustry || "Services",
      aiPrompt: finalPromptForTemplate,
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50/50 text-slate-900 font-sans relative">
      <style dangerouslySetInnerHTML={{ __html: scrollbarStyles }} />

      {/* ══ TOP NAV ══ */}
      <div className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200 px-6 py-3 flex items-center justify-between shadow-2xs">
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate(`/dashboard/projects/${id}`)}
            className="h-8 px-3 text-xs font-semibold inline-flex items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 transition-all shadow-2xs"
          >
            <ArrowLeft className="h-3.5 w-3.5" /> Back
          </button>
          <span className="text-slate-300">/</span>
          {project && (
            <span className="flex items-center gap-2 text-sm text-slate-500 truncate max-w-[200px]">
      ...
      {/* ══ SIMPLE CENTERED PROMPT WORKSPACE ══ */}
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-12 md:py-20 max-w-4xl mx-auto w-full">

        {/* Simple Title */}
        <h1 className="text-3xl md:text-5xl font-black text-slate-900 text-center tracking-tight mb-3">
          What page would you like to create?
        </h1>
        <p className="text-xs md:text-sm text-slate-500 text-center max-w-md mb-6">
          Enter your page name, URL slug, and prompt to generate with AI.
        </p>

        {/* ══ PAGE NAME & SLUG INPUTS ══ */}
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div>
            <label className="text-xs font-semibold text-slate-700 mb-1.5 block text-left">Page Name</label>
            <input
              value={pageName}
              onChange={(e) => {
              ...
              className={`w-full h-10 border rounded-xl px-3.5 text-xs bg-white outline-none transition-all ${pageNameError ? "border-red-400 focus:border-red-500" : "border-slate-200 focus:border-violet-500 focus:ring-2 focus:ring-violet-100"
                }`}
            />
            {pageNameError && <span className="text-red-500 text-[11px] mt-1 block text-left">{pageNameError}</span>}
          </div>

          <div>
            <label className="text-xs font-semibold text-slate-700 mb-1.5 block text-left">URL Slug</label>
            <div className={`flex items-center h-10 border rounded-xl overflow-hidden bg-white transition-all ${slugError ? "border-red-400" : "border-slate-200 focus-within:border-violet-500 focus-within:ring-2 focus-within:ring-violet-100"
              }`}>
              <span className="px-3 h-full flex items-center bg-slate-100 text-xs font-bold text-slate-400 border-r border-slate-200">/</span>
              <input
                value={pageSlug}
                onChange={(e) => {
                ...
                className="flex-1 px-3 text-xs bg-transparent outline-none text-slate-900"
              />
              <div className="px-2 flex-shrink-0">
                {isVerifyingSlug && <Loader2 className="h-3.5 w-3.5 text-violet-500 animate-spin" />}
                {!isVerifyingSlug && isSlugVerified && <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" />}
              </div>
            </div>
            {slugError && <span className="text-red-500 text-[11px] mt-1 block text-left">{slugError}</span>}
          </div>
        </div>

        {/* ══ PROMPT CARD (CLEAN WHITE) ══ */}
        <div className="w-full bg-white border border-slate-200 rounded-2xl shadow-sm focus-within:border-violet-500 focus-within:ring-2 focus-within:ring-violet-100 transition-all p-4 relative mb-6">
          <textarea
            value={aiPrompt}
            onChange={(e) => {
            ...
            placeholder={animatedPlaceholder || "Describe your page..."}
            className="w-full min-h-[130px] bg-transparent text-slate-900 placeholder:text-slate-400 text-sm outline-none resize-none border-none p-0 focus:ring-0 leading-relaxed"
          />

          {/* Prompt Suggestion Chips */}
          {(!showLoader && !createPageMutation.isPending && project) && (
            <div className="flex flex-wrap gap-2 mt-3 mb-1 pt-2 border-t border-slate-50">
              {(() => {
                const CHIP_PROMPTS: Record<string, string[]> = {
                  "Lawyer": [
                    "Create a professional landing page for a personal injury lawyer helping accident victims.",
                    "Create a modern landing page for a family law firm handling divorce and child custody cases.",
                    "Design a high-converting landing page for a criminal defense attorney.",
                    "Create a trusted landing page for an immigration lawyer helping with visas and citizenship.",
                    "Build a professional landing page for a corporate law firm serving businesses and startups.",
                    "Create a landing page for a real estate attorney handling property transactions and disputes.",
                    "Design a landing page for an employment lawyer handling workplace disputes.",
                    "Create a landing page for an estate planning attorney specializing in wills and trusts.",
                    "Build a landing page for a bankruptcy lawyer helping clients with debt relief.",
                    "Create a premium landing page for a law firm offering legal services for individuals and businesses."
                  ],
                  "Plumber": [
                    "Create a professional landing page for an emergency plumbing service.",
                    "Build a modern landing page for a residential plumbing company.",
                    "Design a high-converting landing page for a commercial plumbing contractor.",
                    "Create a trusted landing page for a local plumber specializing in leak repairs.",
                    "Build a landing page for a drain cleaning and sewer repair service."
                  ],
                  "Real Estate": [
                    "Create a modern landing page for a luxury real estate agent.",
                    "Build a professional landing page for a commercial real estate agency.",
                    "Design a high-converting property management company landing page.",
                    "Create a trusted landing page for first-time home buyer real estate services.",
                    "Build a landing page for a real estate investment firm."
                  ],
                  "Medical Industry": [
                    "Create a reassuring landing page for a dental clinic.",
                    "Build a modern landing page for a pediatric healthcare center.",
                    "Design a professional landing page for a physical therapy clinic.",
                    "Create a trusted landing page for a mental health counseling practice.",
                    "Build a premium landing page for a cosmetic surgery center."
                  ],
                  "Book Keeping": [
                    "Create a professional landing page for a small business bookkeeping service.",
                    "Build a modern landing page for a corporate accounting firm.",
                    "Design a high-converting landing page for a tax preparation specialist.",
                    "Create a trusted landing page for a freelance CPA and financial advisor.",
                    "Build a landing page for an outsourced payroll and bookkeeping agency."
                  ]
                };

                const chips = ["Lawyer", "Plumber", "Real Estate", "Medical Industry", "Book Keeping"];

                return chips.map((chip, idx) => (
                  <button
                    key={idx}
                    onClick={() => {
                      const variations = CHIP_PROMPTS[chip] || [`Create a modern landing page for ${chip}.`];
                      const randomPrompt = variations[Math.floor(Math.random() * variations.length)];
                      setAiPrompt(randomPrompt);
                      setActiveChip(chip);
                      setMethodError("");
                    }}
                    className={`px-3 py-1.5 rounded-full text-[11px] font-medium transition-colors border ${activeChip === chip ? 'bg-violet-100 text-violet-700 border-violet-200' : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100'}`}
                  >
                    + {chip}
                  </button>
                ));
          ...
          {((showLoader || createPageMutation.isPending) && showDelayedLoader) ? (
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100 mt-2">
              <button
                ...
                disabled={!aiPrompt.trim() || !pageName.trim() || !pageSlug.trim()}
                className="h-10 px-5 rounded-xl font-bold text-xs text-white bg-violet-600 hover:bg-violet-700 transition-all shadow-sm flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed"
              >
