...
async function replacePlaceholdersInHtml(
  ...
  try {
     ...
     * GENERATE ALL
     */
    const results = await Promise.all(imagesToReplace.map(async (img) => {
      let newUrl;
      if (DISABLE_AI_IMAGES_FOR_TESTING) {
    ...
    if (remainingUrls.length > 0) {
      ...
      const remainingResults = await Promise.all(remainingUrls.map(async (item) => {
        const context = {
          isHero: item.isHero,
        ...
        return { originalUrl: item.url, newUrl };
      }));
