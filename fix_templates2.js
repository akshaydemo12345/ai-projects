...
walkDir(templatesDir, (filePath) => {
    if (filePath.endsWith('.ts')) {
        ...
        if (!content.includes('id="core-interactions"')) {
            // Find the very last backtick of the file
            const lastBacktickIndex = content.lastIndexOf('`');
            if (lastBacktickIndex !== -1) {
                content = content.substring(0, lastBacktickIndex) + '\n' + coreScript + '\n`' + content.substring(lastBacktickIndex + 1);
            }
        }
        
        fs.writeFileSync(filePath, content);
        console.log('Appended script to', filePath);
    }
});
